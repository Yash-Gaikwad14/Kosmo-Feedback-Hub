...
const SUPABASE_STORAGE_BUCKET = process.env.SUPABASE_STORAGE_BUCKET || 'kosmofeedback';
...
// Helper to ensure Supabase Storage Bucket is set to Public for instant URL serving
async function ensureBucketPublic() {
    if (isCloudStorageAvailable && supabase) {
        try {
            const { error } = await supabase.storage.updateBucket(SUPABASE_STORAGE_BUCKET, { public: true });
            if (error) {
                console.warn(`[STORAGE WARNING] Could not set bucket ${SUPABASE_STORAGE_BUCKET} to public:`, error.message);
            } else {
                console.log(`[STORAGE] Verified Supabase Storage bucket '${SUPABASE_STORAGE_BUCKET}' is set to PUBLIC.`);
            }
        } catch (e) {
            console.warn('[STORAGE WARNING] ensureBucketPublic error:', e.message);
        }
    }
}

// Automatically ensure bucket public status on initialization
ensureBucketPublic();
...
async function copyFile(sourceCategory, sourceFilename, destCategory, destFilename) {
    ...
    if (isCloudStorageAvailable) {
        // Download & upload with explicit contentType to ensure correct image headers in Supabase CDN
        ...
        // Fallback to native copy
        const { error: copyErr } = await supabase.storage
            .from(SUPABASE_STORAGE_BUCKET)
            .copy(srcKey, destKey);

        if (copyErr) {
            throw new Error(`Supabase Storage copy error (${srcKey} -> ${destKey}): ${copyErr.message}`);
        }
...
async function deleteFile(categoryKey, filename) {
    const key = `${categoryKey.toLowerCase()}/${filename}`;
    ...
    if (isCloudStorageAvailable) {
        ...
        if (error) {
            console.error(`Error deleting Supabase Storage object ${key}:`, error.message);
        }
    } else {
        const targetDir = categoryKey === 'RAW' ? path.join(BASE_DIR, 'RAW_IMAGES') : path.join(BASE_DIR, categoryKey);
        const fullPath = path.join(targetDir, filename);
...
async function deleteAllCategoryCopies(filename) {
    const categories = ['raw', 'app', 'ui', 'both', 'problems'];

    if (isCloudStorageAvailable) {
        const paths = categories.map(c => `${c}/${filename}`);
        const { error } = await supabase.storage
            .from(SUPABASE_STORAGE_BUCKET)
            .remove(paths);

        if (error) {
            console.error(`Error deleting copies of ${filename}:`, error.message);
        }
    } else {
        categories.forEach(cat => {
            const catDir = cat === 'raw' ? path.join(BASE_DIR, 'RAW_IMAGES') : path.join(BASE_DIR, cat.toUpperCase());
