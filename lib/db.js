...
function loadLocalDb() {
    ...
    return { users: ["Aditya", "Anmol", "Harshal", "Prashik", "Shoaib", "Yash"], problemClusters: [], imageMetadata: {} };
}
    ...
    if (isPostgresAvailable) {
        ...
        try {
            ...
            if (parseInt(userRes.rows[0].count, 10) === 0) {
                const defaultUsers = ["Aditya", "Anmol", "Harshal", "Prashik", "Shoaib", "Yash"];
...
async function getUsers() {
    if (isPostgresAvailable) {
        try {
            ...
            return ["Aditya", "Anmol", "Harshal", "Prashik", "Shoaib", "Yash"];
        }
    } else {
        const local = loadLocalDb();
        return local.users || ["Aditya", "Anmol", "Harshal", "Prashik", "Shoaib", "Yash"];
    }
}
...
async function deleteUser(username) {
    ...
    if (isPostgresAvailable) {
        try {
            await pool.query(`DELETE FROM team_users WHERE username = $1`, [cleanName]);
        } catch (err) {
        ...
        const local = loadLocalDb();
        local.users = (local.users || []).filter(u => u !== cleanName);
        saveLocalDb(local);
        return local.users;
    }
}
...
async function updateUser(oldName, newName) {
    ...
    if (!cleanOld || !cleanNew) return await getUsers();

    if (isPostgresAvailable) {
        try {
            await pool.query(`UPDATE team_users SET username = $1 WHERE username = $2`, [cleanNew, cleanOld]);
        } catch (err) {
        ...
        const local = loadLocalDb();
        if (local.users) {
            const idx = local.users.indexOf(cleanOld);
            if (idx !== -1) {
                local.users[idx] = cleanNew;
                saveLocalDb(local);
            }
        }
        return local.users || [];
...
async function deleteMetadataForFile(filename) {
    if (isPostgresAvailable) {
        try {
            await pool.query(`DELETE FROM image_metadata WHERE filename = $1`, [filename]);
        } catch (err) {
            console.error('Error deleting metadata from Postgres:', err);
        }
    } else {
        const local = loadLocalDb();
        if (local.imageMetadata && local.imageMetadata[filename]) {
            delete local.imageMetadata[filename];
            saveLocalDb(local);
        }
    }
}

async function getAllMetadataMap() {
    if (isPostgresAvailable) {
        try {
            const res = await pool.query(`SELECT * FROM image_metadata`);
            const map = {};
            ...
            console.error('Error fetching all metadata map from Postgres:', err);
        }
    }
    const local = loadLocalDb();
    return local.imageMetadata || {};
}
